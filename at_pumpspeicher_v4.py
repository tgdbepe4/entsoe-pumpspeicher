...
WARUM ÖSTERREICH: Eine akademische Untersuchung der ENTSO-E Transparency
Platform (Hirth/Mühlenpfordt/Bulkeley 2018, siehe Chat-Diskussion) stellt
fest, dass die Datenabdeckung fuer Oesterreich nahezu vollstaendig ist
...
EINSCHRAENKUNG: Auf der Transparency Platform sind die AT-Werte fuer die
Regelzone APG, nicht fuer ganz Oesterreich (einzelne Kraftwerke/Leitungen
gehoeren zu den deutschen Regelzonen TenneT DE / TransnetBW). Fuer die
grossen Speicherkraftwerke (Kaprun, Malta-Reisseck, Kops - alle in
Tirol/Salzburg) sollte das aber keine Rolle spielen.

KEINE AKW-LOGIK: Oesterreich hat kein in Betrieb befindliches
Kernkraftwerk (Zwentendorf wurde nie angefahren) - die Gösgen-artige
Pro-Block-Berechnung aus dem CH-Skript entfaellt hier komplett.
...
def fetch_days(query_fn, days: int) -> pd.DataFrame:
    ...
    # Excel kann keine zeitzonen-bewussten Datetimes - nach Europe/Vienna
    # (lokale Zeit) konvertieren und dann die TZ-Info entfernen.
    result["timestamp"] = (
        result["timestamp"].dt.tz_convert("Europe/Vienna").dt.tz_localize(None)
    )
    return result
...
def main():
    parser = argparse.ArgumentParser(
        description="AT Pumpspeicher Erzeugung + Pumpverbrauch via entsoe-py")
    ...
    parser.add_argument("--out", default=None,
                         help="Output-Pfad fuer --week (Default: "
                              "at_pumpspeicher_woche_JJJJMMTT_HHMM.xlsx)")
    args = parser.parse_args()

    if args.out is None:
        zeitstempel = pd.Timestamp.now(tz="Europe/Vienna").strftime("%Y%m%d_%H%M")
        args.out = f"at_pumpspeicher_woche_{zeitstempel}.xlsx"
    ...
    if args.week:
        ...
        pumped_df = fetch_days(
            lambda s, e: client.query_generation(
                country_code="AT", start=s, end=e, psr_type=PSR_PUMPED_STORAGE),
            args.days,
        )
        ...
        with pd.ExcelWriter(args.out, engine="openpyxl") as writer:
            (pumped_wide if not pumped_wide.empty
             ...
             ).to_excel(writer, sheet_name="Pumpspeicher_AT", index=False)
            writer.sheets["Pumpspeicher_AT"].column_dimensions["A"].width = 20
        ...
        if not pumped_wide.empty:
            ...
            chart_title = (
                f"Pumpspeicher Österreich ({datum_von} – {datum_bis}): "
                f"Turbiniert vs. Hochgepumpt (MW)"
            )
            inject_box_whisker_chart(
                args.out,
                sheet_name="Pumpspeicher_AT",
    ...
    try:
        xml_pumped = client.query_generation(
            country_code="AT", start=start, end=end, psr_type=PSR_PUMPED_STORAGE)
    except Exception as exc:
    ...
    print(f"Pumpspeicher Österreich (Stand ca. {now.strftime('%d.%m.%Y %H:%M UTC')}):\n")
    if pumped_df.empty:
