#!/usr/bin/env python3
"""
Schweiz – Pumpspeicher: Erzeugung UND Pumpverbrauch, via ENTSO-E
================================================================================
Version: v4.1 - Spaltenbreite timestamp (A) automatisch gesetzt, kein manuelles Verbreitern mehr noetig

Schwester-Skript zu at_pumpspeicher.py / de_pumpspeicher.py, hier für die
Schweiz (CH). Fokus: Wie viel Strom wird zum Hochpumpen verbraucht
(direction "out"), nicht nur wie viel beim Turbinieren erzeugt
(direction "in")?

WICHTIGER UNTERSCHIED ZU AT/DE: Bei der Schweiz liefert ENTSO-E die
"out"-Zeitreihe (Pumpverbrauch) NICHT - das wurde im Chat ausführlich
bestätigt (--raw zeigt nirgends outBiddingZone_Domain für CH). Grund
vermutlich: Die Schweiz ist kein EU-Mitglied und fällt deshalb nicht
unter die verbindliche EU-Transparenzverordnung 543/2013, die Österreich
und Deutschland zur vollständigen Meldung verpflichtet. Dieses Skript
dient als direkter Vergleichsfall zu AT/DE - erwartet wird, dass
Hochgepumpt_MW hier leer bleibt oder ganz fehlt.

KEINE AKW-LOGIK: Anders als das ursprüngliche akw_leistung_ch.py-Skript
enthält diese Variante bewusst keine Pro-Block-AKW-Auswertung - reiner
Fokus auf Pumpspeicher, zum direkten Vergleich mit AT/DE.
...
def main():
    parser = argparse.ArgumentParser(
        description="CH Pumpspeicher Erzeugung + Pumpverbrauch via entsoe-py")
    ...
    parser.add_argument("--out", default=None,
                         help="Output-Pfad fuer --week (Default: "
                              "ch_pumpspeicher_woche_JJJJMMTT_HHMM.xlsx)")
    args = parser.parse_args()

    if args.out is None:
        zeitstempel = pd.Timestamp.now(tz="Europe/Zurich").strftime("%Y%m%d_%H%M")
        args.out = f"ch_pumpspeicher_woche_{zeitstempel}.xlsx"
    ...
    if args.week:
        print(f"Hole {args.days} Tage Daten für die Schweiz (tagesweise, dauert evtl. etwas)...")
        ...
        with pd.ExcelWriter(args.out, engine="openpyxl") as writer:
            (pumped_wide if not pumped_wide.empty
             ...
             ).to_excel(writer, sheet_name="Pumpspeicher_CH", index=False)
            # Spalte A (timestamp) ist standardmaessig zu schmal fuer das
            # vollstaendige Datum/Uhrzeit-Format - manuell breiter setzen,
            # damit man sie nicht jedes Mal von Hand ziehen muss.
            writer.sheets["Pumpspeicher_CH"].column_dimensions["A"].width = 20
        ...
        if not pumped_wide.empty:
            ...
            chart_title = (
                f"Pumpspeicher Schweiz ({datum_von} – {datum_bis}): "
                f"Turbiniert vs. Hochgepumpt (MW)"
            )
            inject_box_whisker_chart(
                args.out,
                sheet_name="Pumpspeicher_CH",
    ...
    print(f"Pumpspeicher Schweiz (Stand ca. {now.strftime('%d.%m.%Y %H:%M UTC')}):\n")
    if pumped_df.empty:
