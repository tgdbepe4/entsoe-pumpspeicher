...
Schwester-Skript zu at_pumpspeicher_v4.py, für Frankreich (FR) - und mit Fokus auf genau die Frage, die bei der Schweiz
...
EINSCHRAENKUNG: Frankreich hat mehrere grosse Pumpspeicherwerke (z.B.
Grand'Maison, Montezic, Revin). RTE ist der franzoesische TSO.

KEINE AKW-LOGIK: Kernkraftwerke werden hier nicht ausgewertet - die Gösgen-artige
...
def fetch_days(query_fn, days: int) -> pd.DataFrame:
    ...
    # Excel kann keine zeitzonen-bewussten Datetimes - nach Europe/Paris
    # (lokale Zeit) konvertieren und dann die TZ-Info entfernen.
    result["timestamp"] = (
        result["timestamp"].dt.tz_convert("Europe/Paris").dt.tz_localize(None)
    )
    return result
...
def main():
    parser = argparse.ArgumentParser(
        description="FR Pumpspeicher Erzeugung + Pumpverbrauch via entsoe-py")
    ...
    parser.add_argument("--out", default=None,
                         help="Output-Pfad fuer --week (Default: "
                              "fr_pumpspeicher_woche_JJJJMMTT_HHMM.xlsx)")
    args = parser.parse_args()

    if args.out is None:
        zeitstempel = pd.Timestamp.now(tz="Europe/Paris").strftime("%Y%m%d_%H%M")
        args.out = f"fr_pumpspeicher_woche_{zeitstempel}.xlsx"
    ...
    if args.week:
        ...
        pumped_df = fetch_days(
            lambda s, e: client.query_generation(
                country_code="FR", start=s, end=e, psr_type=PSR_PUMPED_STORAGE),
            args.days,
        )
        ...
        with pd.ExcelWriter(args.out, engine="openpyxl") as writer:
            (pumped_wide if not pumped_wide.empty
             ...
             ).to_excel(writer, sheet_name="Pumpspeicher_FR", index=False)
            writer.sheets["Pumpspeicher_FR"].column_dimensions["A"].width = 20
        ...
        if not pumped_wide.empty:
            ...
            chart_title = (
                f"Pumpspeicher Frankreich ({datum_von} – {datum_bis}): "
                f"Turbiniert vs. Hochgepumpt (MW)"
            )
            inject_box_whisker_chart(
                args.out,
                sheet_name="Pumpspeicher_FR",
    ...
    try:
        xml_pumped = client.query_generation(
            country_code="FR", start=start, end=end, psr_type=PSR_PUMPED_STORAGE)
    except Exception as exc:
    ...
    print(f"Pumpspeicher Frankreich (Stand ca. {now.strftime('%d.%m.%Y %H:%M UTC')}):\n")
    if pumped_df.empty:
