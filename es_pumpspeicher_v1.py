...
EINSCHRAENKUNG: Spanien hat grosse Pumpspeicherkapazitaeten (z.B.
La Muela, Valdecanas). REE ist der spanische TSO (Red Electrica de Espana).
...
def fetch_days(query_fn, days: int) -> pd.DataFrame:
    ...
    # Excel kann keine zeitzonen-bewussten Datetimes - nach Europe/Madrid
    # (lokale Zeit) konvertieren und dann die TZ-Info entfernen.
    result["timestamp"] = (
        result["timestamp"].dt.tz_convert("Europe/Madrid").dt.tz_localize(None)
    )
    return result
...
def main():
    parser = argparse.ArgumentParser(
        description="ES Pumpspeicher Erzeugung + Pumpverbrauch via entsoe-py")
    ...
    parser.add_argument("--out", default=None,
                         help="Output-Pfad fuer --week (Default: "
                              "es_pumpspeicher_woche_JJJJMMTT_HHMM.xlsx)")
    args = parser.parse_args()

    if args.out is None:
        zeitstempel = pd.Timestamp.now(tz="Europe/Madrid").strftime("%Y%m%d_%H%M")
        args.out = f"es_pumpspeicher_woche_{zeitstempel}.xlsx"
    ...
    if args.week:
        ...
        pumped_df = fetch_days(
            lambda s, e: client.query_generation(
                country_code="ES", start=s, end=e, psr_type=PSR_PUMPED_STORAGE),
            args.days,
        )
        ...
        with pd.ExcelWriter(args.out, engine="openpyxl") as writer:
            (pumped_wide if not pumped_wide.empty
             ...
             ).to_excel(writer, sheet_name="Pumpspeicher_ES", index=False)
            writer.sheets["Pumpspeicher_ES"].column_dimensions["A"].width = 20
        ...
        if not pumped_wide.empty:
            ...
            chart_title = (
                f"Pumpspeicher Spanien ({datum_von} – {datum_bis}): "
                f"Turbiniert vs. Hochgepumpt (MW)"
            )
            inject_box_whisker_chart(
                args.out,
                sheet_name="Pumpspeicher_ES",
    ...
    try:
        xml_pumped = client.query_generation(
            country_code="ES", start=start, end=end, psr_type=PSR_PUMPED_STORAGE)
    except Exception as exc:
    ...
    print(f"Pumpspeicher Spanien (Stand ca. {now.strftime('%d.%m.%Y %H:%M UTC')}):\n")
    if pumped_df.empty:
